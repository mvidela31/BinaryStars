...
data {
    ...
    real<lower=0>   plx_obs;   // parallax observation
    real<lower=0>   plx_err;   // parallax observation uncertainty
    real<lower=0>   m1_obs;    // m1 observation
    real<lower=0>   m1_err;    // m1 observation uncertainty
}
transformed data {
...
transformed parameters {
    ...
    real plx = q / (1 + q) * 1 / f_plx;             // parallax [seconds of arc]
...
model {
    ...
    // Parallax prior
    if (plx_obs > 0) {
        plx_obs ~ normal(plx, plx_err);
    }
    // Primary object mass prior
    if (m1_obs > 0) {
        m1_obs ~ normal(m1, m1_err);
    }
}
